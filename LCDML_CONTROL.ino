...
// Modo Joystick
  ...
  #define _LCDML_CONTROL_analog_enter_min        0     // Button Enter
  #define _LCDML_CONTROL_analog_enter_max        100  
  ...
  #define _LCDML_CONTROL_analog_counter_max      3      // vezes antes de entrar ou sair
...
void LCDML_CONTROL_loop()
{
  ...
  if((millis() - g_LCDML_DISP_press_time) >= _LCDML_DISP_cfg_button_press_time) {
    ...
    uint16_t valueZ = analogRead(_LCDML_CONTROL_digital_pin_SW);  // digitalpin for keypad
    ...
    if (valueZ >= _LCDML_CONTROL_analog_enter_min && valueZ <= _LCDML_CONTROL_analog_enter_max) { LCDML_BUTTON_enter(); _LCDML_CONTROL_analog_counter_enter=0; _LCDML_CONTROL_analog_counter_quit=0; sleep_timer_counter=millis();u8g.sleepOn(); analogWrite(backlight, brilho); Serial.print("Value Z:"); Serial.println(valueZ);}
